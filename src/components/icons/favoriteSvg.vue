...
interface IProps{
  people: IPeople | IFormattedPeople
}
const store = useStore()

const {people} = defineProps<IProps>()

const favoriteList = computed(() => store.favoriteList)
const isActive = computed(() => !!favoriteList.value.some((el) => el.name === people.name))

</script>

<style lang="sass" scoped>
svg
  @apply transition-all duration-300 cursor-pointer w-[24px] h-[24px]
  @apply lg:w-[32px] lg:h-[32px]
  &:hover
    fill: #5E8AE7
</style>